...
_glflareyellow = "1Rnd_Smoke_M203"; _glsmokegreen = "1Rnd_SmokeGreen_M203"; _glsmokered = "1Rnd_SmokeRed_M203";    							// Smoke for FTLs, Squad Leaders, etc
...
// LOADOUT: COMMANDER
	case "co":
	{
		...
		{_unit addmagazine _glflareyellow} foreach [1,2,3];
		...
		{_unit addMagazine _strobe} forEach [1,2];
...
// LOADOUT: FIRE TEAM LEADER
	case "ftl":
	{
		...
		{_unit addmagazine _glflareyellow} foreach [1,2,3,4];
		...
		if (_useBackpacks==1) then {
			...
			clearMagazineCargoGlobal (unitBackpack _unit);
			(unitBackpack _unit) addMagazineCargoGlobal [_glriflemag, 4];
			(unitBackpack _unit) addMagazineCargoGlobal [_glflareyellow, 1];
...
// LOADOUT: GRENADIER
	case "gren":
	{
		...
		{_unit addmagazine _glflareyellow} foreach [1,2];
		{_unit addmagazine _grenade} foreach [1,2];
		{_unit addmagazine _smokegrenade;} foreach [1,2];
		if (_useBackpacks==1) then {
			_unit addBackpack _bagmedium;
...
// CARGO: CAR - room for 10 weapons and 50 cargo items
	case "v_car":
	{
		...
		_unit addMagazineCargoGlobal [_glflareyellow, 4];
	};
...
// CARGO: TRUCK - room for 50 weapons and 200 cargo items
	case "v_tr":
	{
		...
		_unit addMagazineCargoGlobal [_glflareyellow, 24];
	};
